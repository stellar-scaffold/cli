...
The `npm run dev` command should have finished building the contracts by now so you can open the app in your browser. It should be running at Vite's default port, [http://localhost:5173](http://localhost:5173), and you should see the home page:
...
This is the Docker container running Stellar locally. You can manage it with `stellar container` commands, but Stellar Scaffold should handle everything for you. We recommend using the local network for development because it's simpler than working with [the other networks](https://developers.stellar.org/docs/learn/fundamentals/networks), but we'll make use of them in later parts of the tutorial.
...
Stellar Scaffold creates an account for you to use on the local network and funds it so you can actually deploy your contracts. You can manage these accounts with the `stellar keys` command.
...
3. We ran `npm run dev` to build and deploy the contracts to our local network, then run the application
